...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    private AudioProcessor[] getEpicenterAudioProcessors() {
        Log.d(TAG, "Epicenter AudioProcessor injected");
        return new AudioProcessor[] { EpicenterProcessorController.INSTANCE.getProcessor() };
    }
    private void ensurePlayerInitialized() {
        if (player == null) {
            RenderersFactory renderersFactory = (eventHandler, videoListener, audioListener, textOutput, metadataOutput) -> {
                DefaultRenderersFactory epicenterRenderersFactory = new DefaultRenderersFactory(context) {
                    @Override
